...
export type DepartureGroupItem = {
  id: string
  room_number: string
...
export function groupDepartureItems(items: DepartureGroupItem[]) {
  ...
  for (const item of items) {
    ...
    if (!currentGroup) {
      ...
      groups.set(groupId, {
        ...
        occupancy_status: item.occupancy_status === 'checked_out' ? 'checked_out' : 'checked_in',
    ...
    currentGroup.occupancy_status = currentGroup.items.every((groupItem) => groupItem.occupancy_status === 'checked_out')
      ? 'checked_out'
      : 'checked_in'
...
export function allocateDeparturePayment(
  items: DepartureGroupItem[],
  cashAmount: number,
  cardAmount: number
) {
